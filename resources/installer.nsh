!macro customCheckAppRunning
  ...
  nsExec::Exec 'taskkill /T /F /IM "MEP Lab.exe"'
  Pop $R0
  ...
  nsExec::Exec 'powershell.exe -NoProfile -Command "Get-CimInstance Win32_Process | Where-Object { $$_.Name -eq ''node.exe'' -and ($$_.ExecutablePath -like ''*MEP Lab*'' -or $$_.ExecutablePath -like ''*Open Cowork*'') } | ForEach-Object { Stop-Process -Id $$_.ProcessId -Force -ErrorAction SilentlyContinue }"'
  Pop $R0
...
Function MepLabShowLegacyUninstallHelp
  Exch $0
  DetailPrint `MEP Lab legacy Open Cowork uninstall failed: $0`

  ; ── installer pass (BUILD_UNINSTALLER is NOT defined) ──────────────
  !ifndef BUILD_UNINSTALLER
    MessageBox MB_OK|MB_ICONEXCLAMATION "MEP Lab could not remove the previously installed legacy Open Cowork version.$\r$\n$\r$\nThis usually means the legacy Windows uninstaller is damaged.$\r$\n$\r$\nNext steps:$\r$\n1. Close all MEP Lab and legacy Open Cowork windows.$\r$\n2. Run:$\r$\n$EXEDIR\Open-Cowork-Legacy-Cleanup.cmd$\r$\n3. Start the MEP Lab installer again.$\r$\n$\r$\nAdd -RemoveAppData to the cleanup tool only if you also want to clear local settings."
  !endif

  ; ── uninstaller pass (BUILD_UNINSTALLER is defined) ────────────────
  !ifdef BUILD_UNINSTALLER
    MessageBox MB_OK|MB_ICONEXCLAMATION "MEP Lab could not remove the previously installed legacy Open Cowork version.$\r$\n$\r$\nThis usually means the legacy Windows uninstaller is damaged.$\r$\n$\r$\nPlease close MEP Lab and legacy Open Cowork, delete:$\r$\n$LOCALAPPDATA\Programs\Open Cowork$\r$\nand then run the MEP Lab installer again.$\r$\n$\r$\nLocal settings may remain in AppData by design."
  !endif
...
!macro customUnInstallCheck
  ${if} $R0 != 0
    Push $R0
    Call MepLabShowLegacyUninstallHelp
  ${endif}
!macroend
